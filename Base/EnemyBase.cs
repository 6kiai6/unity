...
public abstract class EnemyBase : MonoBehaviour, IstateMachineOwner
{
    ...
    [Tooltip("任务统计用：0 表示任意任务（仅统计不过滤类型）；非 0 与 QuestDefinition.enemyTypeIdFilter 对应")]
    public int questEnemyTypeId;
    [Tooltip("血条预制体")]
    ...
    private void NotifyEnemyKilledForQuests()
    {
        ...
        {
            intArg = questEnemyTypeId,
            stringArg = gameObject.tag
        };
        hub.TriggerEvent(GameEventNames.EnemyKilled, payload);
    }
